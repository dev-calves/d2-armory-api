  ...
  describe('401 errors', () => {
    ...
    test('should send 401 errors to an error-handler when no tokens are available', async (done) => {
      ...
      response.on('end', function () {
        ...
        expect(routeResponse.message).toEqual('Please login to use this service.')
        done()
      })
    ...
    test('should refresh tokens when the refresh header is available', async (done) => {
      jest.setTimeout(15000) // this test needs more time because of the multiple async events.
